...
export function bindWindowControlsBridge(): void {
  registerBridgeCommand<void, WindowControlsState>(GET_STATE_CHANNEL, async (_payload, event) => {
    const targetWindow = resolveSenderWindow(event.sender);
    return buildWindowState(targetWindow);
  });

  registerBridgeCommand<void, WindowControlsState>(MINIMIZE_CHANNEL, async (_payload, event) => {
    const targetWindow = resolveSenderWindow(event.sender);
    targetWindow.minimize();
    return buildWindowState(targetWindow);
  });

  registerBridgeCommand<void, WindowControlsState>(TOGGLE_MAXIMIZE_CHANNEL, async (_payload, event) => {
    const targetWindow = resolveSenderWindow(event.sender);
  ...
  registerBridgeCommand<void, null>(CLOSE_CHANNEL, async (_payload, event) => {
    const targetWindow = resolveSenderWindow(event.sender);
    targetWindow.close();
    return null;
  });
