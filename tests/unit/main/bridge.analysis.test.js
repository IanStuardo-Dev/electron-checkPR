...
const analysisShared = require('../../../src/modules/runtime-host/application/analysis/services/analysis-input-normalizers.service');
...
const {
  ...
  bindAnalysisBridge,
} = require('../../../src/modules/runtime-host/presentation/adapters/analysis-adapter');
...
describe('analysis bridge', () => {
  ...
  test('bindAnalysisBridge registra run y cancel', async () => {
    ...
    expect(registerBridgeCommand).toHaveBeenCalledTimes(6);
    const previewHandler = registerBridgeCommand.mock.calls[0][1];
    const runHandler = registerBridgeCommand.mock.calls[1][1];
    const cancelHandler = registerBridgeCommand.mock.calls[2][1];
    const prAiPreviewHandler = registerBridgeCommand.mock.calls[3][1];
    const prAiHandler = registerBridgeCommand.mock.calls[4][1];
    const prAiCancelHandler = registerBridgeCommand.mock.calls[5][1];
  ...
  test('createAnalysisOperations encapsula sanitizacion y resolucion de api key', async () => {
    const repositoryAnalysisService = {
      previewSnapshot: jest.fn().mockResolvedValue({ repository: 'repo-a' }),
      runAnalysis: jest.fn().mockResolvedValue({ summary: 'ok' }),
      cancelAnalysis: jest.fn(),
    ...
    const handlers = createAnalysisOperations(
      repositoryAnalysisService,
      pullRequestAnalysisService,
