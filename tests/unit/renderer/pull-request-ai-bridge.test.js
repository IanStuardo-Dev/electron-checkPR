const pullRequestAiBridge = require('../../../src/renderer/features/pull-request-ai/data/pullRequestAiBridge');

describe('pull request ai bridge', () => {
  beforeEach(() => {
    global.window = {
      electronApi: {
        invoke: jest.fn(),
  ...
  test('preview usa el canal correcto', async () => {
    ...
    const result = await pullRequestAiBridge.previewPullRequestAiReviews(payload);
  ...
  test('preview propaga errores de main', async () => {
    ...
    await expect(pullRequestAiBridge.previewPullRequestAiReviews({ items: [] })).rejects.toThrow('preview failed');
  });

  test('run propaga error si main falla', async () => {
    window.electronApi.invoke.mockResolvedValue({ ok: false, error: 'quota' });
    await expect(pullRequestAiBridge.runPullRequestAiReviews({ items: [] })).rejects.toThrow('quota');
  });
  ...
  test('run devuelve reviews cuando main responde ok', async () => {
    ...
    await expect(pullRequestAiBridge.runPullRequestAiReviews({ items: [] })).resolves.toEqual([{ pullRequestId: 1, status: 'analyzed' }]);
  });

  test('cancel usa el canal correcto', async () => {
    window.electronApi.invoke.mockResolvedValue({ ok: true });
    await expect(pullRequestAiBridge.cancelPullRequestAiReviews('req-1')).resolves.toBeUndefined();
  ...
  test('cancel propaga error si no puede abortar', async () => {
    ...
    await expect(pullRequestAiBridge.cancelPullRequestAiReviews('req-2')).rejects.toThrow('cancel failed');
  });
});
